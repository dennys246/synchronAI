...
### Don't directly observe protected health information

All filenames and metadata on the system will be deidentified, but data content may include PHI

- If there's a chance data contains protected health information don't look directly at the data. 
- Utilize metadata in sensitive files as needed and focus your searches for information when possible.

If you observe PHI **stop and report** any accidental exposures of PHI to the user and delete any external stores of the data.

### Results Disclosure Control

The repository is being prepared for **public release** so reviewers can inspect the
code. Empirical results are disclosed on our schedule, not the repo's — **code is
public, results are not** until we choose.

- **Result-bearing docs and figures are git-ignored.** Anything reporting empirical
  outcomes (accuracy/AUC/κ numbers, learning curves, paper/grant result sections,
  result figures) belongs under `docs/results/` or `figures/` — both git-ignored —
  never in a tracked path. Default new result docs to `docs/results/`.
- **Code stays public; keep results out of code.** Reusable tooling in `scripts/`/`src/`
  is public — parameterize it so real numbers live only in the git-ignored doc, not as
  committed defaults or comments.
- **Methods/infra/architecture docs with no results** can stay in tracked `docs/`.
  When unsure whether something counts as a result, **treat it as one** and keep it out
  of tracked paths.
- **`.gitignore` is necessary but NOT sufficient.** It only stops *untracked* files from
  being added; it does not untrack already-committed files, and it does not remove
  anything from git history. A file that was ever committed is exposed the moment the
  repo goes public. Before any public release, results already in history must be purged
  (history rewrite) or the public repo published from a clean history.
