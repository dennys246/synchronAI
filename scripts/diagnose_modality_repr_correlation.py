...
def compute_corr(video_repr: np.ndarray, audio_repr: np.ndarray) -> dict:
    """Compute four correlation measures between video and audio representations.
    ...
    # 2. Per-dim Pearson on matched dimensions (dim i video vs dim i audio).
    v_centered = video_repr - video_repr.mean(0, keepdims=True)
    a_centered = audio_repr - audio_repr.mean(0, keepdims=True)
    v_std = video_repr.std(0) + 1e-12
    a_std = audio_repr.std(0) + 1e-12
    per_dim_corr = (v_centered * a_centered).mean(0) / (v_std * a_std)
    mean_per_dim_corr = float(per_dim_corr.mean())

    # 3. Linear R² (video → audio): how well audio_repr is linearly explained by video_repr.
    W_va, *_ = np.linalg.lstsq(video_repr, audio_repr, rcond=None)
    audio_pred = video_repr @ W_va
    ss_res = ((audio_repr - audio_pred) ** 2).sum()
    ss_tot = ((audio_repr - audio_repr.mean(0, keepdims=True)) ** 2).sum()
    r2_v_to_a = float(1 - ss_res / max(ss_tot, 1e-12))

    # 4. Linear R² (audio → video): the reverse direction.
    W_av, *_ = np.linalg.lstsq(audio_repr, video_repr, rcond=None)
    video_pred = audio_repr @ W_av
    ss_res = ((video_repr - video_pred) ** 2).sum()
    ss_tot = ((video_repr - video_repr.mean(0, keepdims=True)) ** 2).sum()
    r2_a_to_v = float(1 - ss_res / max(ss_tot, 1e-12))

    return {
        "n_val_samples": n,
        "proj_dim": p,
        "mean_cosine_sim": mean_cos,
        "mean_per_dim_pearson": mean_per_dim_corr,
...
def inspect_checkpoint(ckpt_path: Path) -> dict:
    ...
    if num_folds and num_folds > 0 and fold_idx is not None and fold_idx >= 0:
        _, val_entries = train_mm.subject_kfold_split(entries, num_folds, fold_idx, config["seed"])
    else:
        _, val_entries = train_mm.subject_grouped_split(entries, config["val_split"], config["seed"])
...
def print_one(name: str, s: dict) -> None:
    print(f"  {name:<14}  arch={s['arch']:<3} P={s['proj_dim']:<3} N_val={s['n_val_samples']:<5}  "
          f"cos={s['mean_cosine_sim']:>+.4f}  per-dim Pearson={s['mean_per_dim_pearson']:>+.4f}  "
