...
def load_and_normalize_recording(
    ...
    try:
        ...
        # Optional extra low-pass to strip the respiration band (confound test):
        # the model's strongest single-band cue was 0.2-0.4 Hz residual; cutting
        # at ~0.15 Hz removes it while keeping task (0.01-0.05) and Mayer (~0.1).
        if lowpass_hz is not None:
            from scipy.signal import butter, filtfilt
            fs = float(raw.info["sfreq"])
            wn = min(lowpass_hz / (0.5 * fs), 0.99)
            b, a = butter(4, wn, btype="low")
            x = filtfilt(b, a, x, axis=0).astype(np.float32)
        # Optional high-pass to KEEP the respiration band (0.15+ Hz) — the clean
        # complement of the low-pass, for the band-decomposition (respiration-only).
        if highpass_hz is not None:
            from scipy.signal import butter, filtfilt
            fs = float(raw.info["sfreq"])
            wn = max(highpass_hz / (0.5 * fs), 1e-4)
...
def extract_features(
    ...
    random_seed: int = 42,
    random_encoder_out: Optional[str] = None,
    lowpass_hz: Optional[float] = None,
    highpass_hz: Optional[float] = None,
    ...
    if random_init:
        # Seed BEFORE instantiation so the random initialization is reproducible.
    ...
    if random_init:
        logger.info("ABLATION MODE: randomly initialized encoder (seed=%d)", random_seed)
        if random_encoder_out:
            # Save in the format the loader expects (encoder_config + config +
            # state_dict) so downstream IG can load it via --encoder-weights.
            Path(random_encoder_out).parent.mkdir(parents=True, exist_ok=True)
            torch.save({"encoder_config": encoder_config, "config": diffusion_config,
                        "state_dict": encoder.state_dict()}, random_encoder_out)
            logger.info("Saved random encoder → %s", random_encoder_out)
    else:
    ...
    # Window-length override (re-windowing experiment): the encoder is fully
    # convolutional, so it accepts any window length. real_len = round(window*fs);
    # model_len rounds up to a multiple of 2**depth so the strided down-path is clean.
    if window_seconds is not None:
        real_len = int(round(window_seconds * target_sfreq))
        factor = 2 ** int(encoder_config.get("depth", 3))
        model_len = ((real_len + factor - 1) // factor) * factor
        duration_seconds = float(window_seconds)
        logger.info(
            "WINDOW OVERRIDE: window_seconds=%s -> real_len=%d model_len=%d (factor=%d)",
            window_seconds, real_len, model_len, factor,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Seed for the --random-init encoder, for reproducibility (default 42).",
    )
    parser.add_argument(
        "--lowpass-hz", type=float, default=None,
        help="Optional extra low-pass cutoff (Hz) applied before encoding, e.g. 0.15 "
             "to strip the respiration band for the confound-removal test.",
    )
    parser.add_argument(
        "--highpass-hz", type=float, default=None,
        help="Optional extra high-pass cutoff (Hz) applied before encoding, e.g. 0.15 "
             "to KEEP only the respiration band (respiration-only band-decomposition model).",
    )
    parser.add_argument(
        "--window-seconds", type=float, default=None,
        help="Override the encoder window length (seconds) for the re-windowing "
             "experiment. The encoder is fully convolutional, so it accepts any "
             "window; real_len=round(window*fs), model_len rounds up to a multiple "
             "of 2**depth. Set --stride-seconds equal to this for non-overlapping tiles.",
    )
    parser.add_argument(
        "--random-encoder-out", default=None,
        help="If set with --random-init, save the random encoder weights here "
             "(loadable format) so downstream IG can run through it.",
    )
    parser.add_argument(
    ...
    extract_features(
        ...
        random_encoder_out=args.random_encoder_out,
