...
def pack_features(
    ...
    # Append row_idx across ALL n rows first so every offset stays aligned with
    # the packed bin, THEN drop corrupt rows from the INDEX. Otherwise training
    # loads a zero-filled feature row paired with a real label (silent label
    # noise in whichever split it lands in). The zero rows remain in the bin to
    # preserve every surviving row's offset; the memmap is sized from
    # features_meta.json (n rows), not the index length, so a shorter index is safe.
    df["row_idx"] = range(n)
    if corrupt_files:
        df = df[~df["feature_file"].isin(set(corrupt_files))].reset_index(drop=True)
        logger.warning(
            "Excluded %d corrupt row(s) from %s (zero rows kept in bin for "
            "alignment; entries removed so training never loads them)",
            len(corrupt_files), index_path.name,
        )
...
def split_fnirs_feature_entries(
    ...
    produced a DIFFERENT train/val split in every process even with a fixed
    seed — which meant a model trained in one process and re-evaluated in
    another was scored on a leaky val set (overlapping its own training
    subjects). Sorting first makes the shuffle input deterministic, so the
...
def filter_by_window_idx(
    entries: list[dict], window_idx_filter: str | None,
) -> list[dict]:
    """Filter entries by their `window_idx` column.

    Format: '<op>:<int>' where op ∈ {eq, gt, ge, lt, le}.
    e.g. 'eq:0' → only window 0 of each pair;  'gt:0' → all later windows.
    Pass None or empty string to skip filtering.
    """
    if not window_idx_filter:
        return entries

    op, _, val_s = window_idx_filter.partition(":")
    if not val_s:
        raise ValueError(
            f"window_idx_filter must be '<op>:<int>' (got '{window_idx_filter}')"
        )
    val = int(val_s)
    ops = {
        "eq": lambda x: x == val,
    ...
    if op not in ops:
        raise ValueError(f"unknown op '{op}'; choose from {sorted(ops)}")

    keep = ops[op]
    before = len(entries)
    filtered = [e for e in entries if keep(int(e.get("window_idx", -1)))]
    logger.info(
        "window_idx filter (%s %d): %d → %d entries",
        op, val, before, len(filtered),
    )
    return filtered
...
def create_fnirs_feature_dataloaders(
    ...
    entries = filter_by_window_idx(entries, window_idx_filter)
    if not entries:
        raise ValueError(
            f"No entries remaining after window-idx filter "
            f"(window_idx_filter={window_idx_filter})."
        )
