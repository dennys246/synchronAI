...
def _generate_sample(
    ...
    # Derive n_pairs from feature_dim, NOT len(pair_names): per-pair models
    # (feature_dim=2) emit ONE pair even though pair_names lists the full montage,
    # so len(pair_names)=10 would demand 20 columns and raise on reshape (the
    # error is swallowed warn-only below, silently killing sample plots).
    # (20-ch: 20//2=10, unchanged.) Matches inference/fnirs/generate.py.
    n_pairs = config.feature_dim // hb_count
